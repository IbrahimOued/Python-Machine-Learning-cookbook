# This is a lengthy recipe, so we will only look at the important functions.
# The full code is given in the build_features.py file that is already
# provided for you.
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description='Extract features from a given set of images')
    ...
    parser.add_argument("--scaling-size", dest="scaling_size", type=int,
                        default=200, help="Scales the longer dimension of the image down to this size.")
...
# 1 Let's look at the class that's defined to extract features:


class FeatureBuilder(object):
    # 2 Define a method to extract features from the input image.
    # We will use the Star detector to get the keypoints and then
    # use SIFT to extract descriptors from these locations:
    ...
    def get_codewords(self, input_map, scaling_size, max_samples=12):
        ...
        count = 0
        cur_class = ''
        # 4 Each image will give rise to a large number of descriptors.
        # We will just use a small number of images because the centroids won't change much after this:
...
if __name__ == '__main__':
    ...
    feature_map = get_feature_map(
        training_data, kmeans, centroids, scaling_size)
    if args.feature_map_file:
        with open(args.feature_map_file, 'wb') as f:
            pickle.dump(feature_map, f)
# python ch10/build_features.py --data-folder ch10/training_images/ --codebook-file codebook.pkl --feature-map-file feature_map.pkl
